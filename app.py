# app.py – JOVAL WINES RISK PORTAL v16.4 – FULL NIST + AUDIT TRAIL
...
def init_db():
    ...
    # === COMPLETE NIST CSF 2.0 CONTROLS (50+) ===
    nist_full = [
        # GOVERN (GV)
    ...
    c.executemany("INSERT OR IGNORE INTO risks (company_id, title, description, category, likelihood, impact, status, submitted_by, submitted_date, risk_score, approver_email, approver_notes) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", risks)
...
<style>
    ...
    .metric-card {background: white; padding: 1.5rem; border-radius: 12px; text-align: center;}
</style>
""", unsafe_allow_html=True)

st.markdown('<div class="header"><h1>JOVAL WINES</h1><p>Risk Management Portal v16.4</p></div>', unsafe_allow_html=True)
    ...
    if st.session_state.get("selected_risk"):
        ...
        with st.form("edit"):
            ...
            if st.form_submit_button("Update"):
                ...
                conn.commit()
                log_action(user[1], "RISK_UPDATED", f"ID: {risk[0]}, Status: {status}")
                st.success("Updated")
        ...
        with st.form("new"):
            ...
            if st.form_submit_button("Submit"):
                ...
                cid = c.fetchone()[0]
                c.execute("INSERT INTO risks (company_id, title, description, category, likelihood, impact, status, submitted_by, submitted_date, risk_score, approver_email, approver_notes) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
...
# === NIST CONTROLS (FULL LIST) ===
elif page == "NIST Controls":
    st.markdown("## NIST CSF 2.0 Controls (50+)")
...
# === AUDIT TRAIL ===
    ...
    st.download_button("Download Audit Log", csv, "audit_trail.csv", "text/csv")

# === OTHER PAGES (Evidence, Playbooks, Reports, Vendor, Admin) ===
    ...
    for pb in pbs["playbook_name"]:
        with st.expander(pb):
            ...
            for _, s in steps.iterrows():
                ...
                with col2: st.text_input("", s["notes"], key=f"note_{s['id']}")
