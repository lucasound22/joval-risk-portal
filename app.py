...
# ==========================================
# 2. DATABASE MANAGEMENT (Direct Connection)
# ==========================================
def get_connection():
...
def nuke_and_reset_db():
    """
    CRITICAL FUNCTION: Wipes the database and re-seeds it with correct IDs.
    This fixes the 'Unknown' company bug.
    """
    ...
    # Retrieve the specific IDs generated by SQLite to ensure foreign key integrity
    company_map = {}
    for comp in companies:
        c.execute("SELECT id FROM companies WHERE name=?", (comp,))
        company_map[comp] = c.fetchone()[0]

    hashed = hashlib.sha256("Joval2025".encode()).hexdigest()
    
    # Create Users linked to the correct Company IDs
    for comp_name, comp_id in company_map.items():
        # Admin User
        admin_email = f"admin@{comp_name.lower().replace(' ', '')}.com.au"
        c.execute("INSERT OR IGNORE INTO users (username, email, password, role, company_id) VALUES (?, ?, ?, ?, ?)", 
                  ("admin", admin_email, hashed, "Admin", comp_id)) 
        
        # Approver User
        approver_email = f"approver@{comp_name.lower().replace(' ', '')}.com.au"
        c.execute("INSERT OR IGNORE INTO users (username, email, password, role, company_id) VALUES (?, ?, ?, ?, ?)", 
                  (f"approver_{comp_name.lower().replace(' ', '')}", approver_email, hashed, "Approver", comp_id))
...
def generate_pdf_report(title, content):
    ...
    # Try to add logo, fallback if fails
    try:
...
if "user" not in st.session_state:
    with st.sidebar:
        ...
        if st.button("⚠️ HARD RESET DB (Use if broken)", type="primary"):
            nuke_and_reset_db()
            st.success("Database Reset! Please try logging in.")
...
with st.sidebar:
    ...
    # Warning if data corruption detected
    if user_company_name == "Unknown":
        st.error("❌ Company ID Error. Please Reset DB.")
        if st.button("Fix Account Now"):
            nuke_and_reset_db()
...
if page == "Dashboard":
    ...
    # Admin View Logic: Allows seeing other companies
...
# ==========================================
    ...
    # 2. Select Company
    c_name = st.selectbox("Company", comps['name'].tolist())
    ...
    if approvers.empty:
        st.error(f"No approvers found for {c_name}. Please go to Admin Panel and create an Approver for this company.")
        st.stop()
        
    approver = st.selectbox("Approver", approvers['email'].tolist())
    ...
    with st.form("risk_form"):
        ...
        if st.form_submit_button("Submit Risk"):
            if not title or not desc:
                ...
                if rows > 0:
                    st.success(f"Risk Logged Successfully for {c_name}!")
                    send_email(approver, "New Risk", f"Risk: {title}")
                    st.session_state.page = "Dashboard"
                    st.rerun()
                else:
                    st.error("Database Write Failed. Please check logs.")
...
# ==========================================
    ...
    # Only show reports for the user's company (or all if admin)
    target_id = user[5]
    df = run_query("SELECT * FROM risks WHERE company_id=?", (target_id,))
    
    st.dataframe(df)
    if not df.empty:
        pdf = generate_pdf_report("Risk Report", df)
...
# ==========================================
    ...
    with st.expander("Add User"):
        with st.form("new_u"):
            ...
            c_df = run_query("SELECT id, name FROM companies")
            c_name = st.selectbox("Company", c_df['name'].tolist())
            c_id = c_df[c_df['name'] == c_name].iloc[0]['id']
            
            if st.form_submit_button("Create"):
                h = hashlib.sha256(p.encode()).hexdigest()
                res = run_query("INSERT INTO users (username, email, password, role, company_id) VALUES (?, ?, ?, ?, ?)",
                          (u, e, h, r, c_id), is_write=True)
                if res:
                    st.success("Created")
                else:
                    st.error("Failed. User might exist.")

    st.write("### System Users")
    users_view = run_query("SELECT id, username, email, role, company_id FROM users")
    st.dataframe(users_view)
