...
def init_db():
    ...
    risks = [
        ...
        (3, "Suspicious Login from Russia", "MFA bypass attempt on CEO account at 3 AM", "IDENTIFY", "High", "Medium", "Pending Approval", "ciso@bnv.com.au", "2025-10-03", 6, "approver@bnv.com.au", ""),
        (4, "Vendor Portal Exposed to Public", "Shodan scan revealed open admin interface", "PROTECT", "High", "High", "Open", "security@bam.com.au", "2025-10-02", 9, "approver@bam.com.au", "")
    ]
...
if page == "Dashboard":
    ...
    for _, r in risks.iterrows():
        ...
        comp = c.fetchone()[0]
        if st.button(f"{r['title']} [{comp}] - {r['status']} (Score: {r['risk_score']})", key=f"risk_{r['id']}"):
